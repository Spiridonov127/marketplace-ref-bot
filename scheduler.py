...
def clean_search_query(query: str) -> str:
    ...
    q = query or ""
    # "5 лучших наушников" — без "топ" число не попадает под _QUERY_NOISE_RE
    # (там нет цифр в альтернативах) и остаётся в тексте поиска после того,
    # как "лучших" вырежется: "5 Беспроводных наушников" — с такой "5" в
    # начале Маркет ищет плохо или вообще ничего не находит (баг найден на
    # живом запросе). Ведущее 1-2-значное число — почти всегда просто счётчик
    # ("5 лучших", "10 подборка"), а не часть названия товара, поэтому его
    # можно смело срезать ещё до общего прохода по служебным словам.
